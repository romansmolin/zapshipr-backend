...
exports.BullMqPostWorker = void 0;
...
class BullMqPostWorker {
    constructor(platform, postInteractor) {
        this.platform = platform;
        this.config = platform_config_1.PlatformConfigManager.getConfig(platform);
        ...
        const limiter = this.limitsEnabled
            ...
            : undefined;
        this.worker = new bullmq_1.Worker(`${platform}-scheduler`, async (job) => this.handleJob(job), {
    ...
    setOnJobFailureCallback(callback) {
    ...
    customBackoffStrategy(attemptsMade, err, job) {
        ...
        if (!jitter_1.RetryUtils.isRetryableError(err)) {
        ...
        const errorRetryAfter = jitter_1.RetryUtils.extractRetryAfter(err);
        if (errorRetryAfter) {
        ...
        const retryDelay = jitter_1.JitterUtils.calculateRetryDelay(this.platform, attemptsMade, postId, userId);
        console.log(`[${this.platform}] Calculated retry delay: ${retryDelay}ms for job ${job.id}, attempt ${attemptsMade}`);
        return retryDelay;
    }
    start() {
        ...
        this.worker.on('failed', (job, err) => {
            if (!job)
                return;
            const { postId, userId } = job.data;
            const isRetryable = jitter_1.RetryUtils.isRetryableError(err);
    ...
    async handleJob(job) {
        ...
        try {
            ...
            if (this.limitsEnabled && !rateLimitResult.allowed) {
                ...
                const error = jitter_1.RetryUtils.createRetryableError(new Error(`Rate limit exceeded. Retry after ${retryAfter} seconds`), true, retryAfter);
            ...
            const isRetryable = jitter_1.RetryUtils.isRetryableError(error);
            const retryAfter = jitter_1.RetryUtils.extractRetryAfter(error);
            // Convert to retryable error if not already
            const retryableError = isRetryable
                ? error
                : jitter_1.RetryUtils.createRetryableError(error, false);
    ...
    // ========== HELPER METHODS ==========
    async updateJobStatus(job, status, message) {
        try {
    ...
    emitMetrics(type, duration, remainingQuota, errorMessage) {
    ...
    async getQuotaStatus(platform, userId) {
        ...
        // Generate warnings
        if (limits.warnThresholdPct &&
            limits.appDailyLimit &&
            perAppResult.remainingQuota < limits.appDailyLimit * limits.warnThresholdPct) {
            warnings.push(`App quota usage at ${Math.round((1 - perAppResult.remainingQuota / limits.appDailyLimit) * 100)}%`);
        }
    ...
    async checkAppQuotaStatus(platform, limits, now) {
...
exports.BullMqPostWorker = BullMqPostWorker;
